...
The Saqrmisc package includes eight main functional areas:
...
6. **Model-Based Clustering** (`clustering`)
7. **Categorical Variable Analysis** (`mosaic_analysis`)
8. **Network Estimation** (`estimate_single_network`)
...
# Basic correlation matrix (bivariate/zero-order)
correlation_matrix(
  data = mtcars,
...
- **Correlation types**: Bivariate (zero-order), partial, semi-partial
...
- **Confidence intervals**: Optional 95% CIs (bivariate Pearson only)
...
# Full correlation table with all statistics
correlations(
  data = mtcars,
  Vars = c("mpg", "cyl", "disp", "hp", "wt")
)
...
- **Full statistics**: r, 95% CI, t-statistic, df, p-value, n for each pair
- **Correlation types**: Bivariate, partial, semi-partial
- **Filtering**: Show only strong (`min_r`) or significant (`sig_only`) correlations
- **P-value adjustment**: Bonferroni, Holm, FDR correction
- **Group-by**: Compute correlations separately per group level
- **Multilevel**: Within-cluster correlations for nested/repeated measures data
- **Between correlations**: Optional between-cluster correlations
- **Color highlighting**: Strong positive (blue), strong negative (red), significant (green)
- **Sorted output**: Results sorted by absolute correlation strength
